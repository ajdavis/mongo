...
size_t getFrames(frame* frames, std::ostream& logStream) {
    size_t nFrames = 0;
    frame* pFrame = frames;
    unw_cursor_t cursor;
    ...
    MONGO_UNWIND_CHECK(unw_init_local, &cursor, &context);
 ...
 *
 * Notes for future refinements: we have a goal of making this malloc-free so
 * it's signal-safe. This huge stack-allocated structure reduces the need for
 * malloc, at the risk of a stack overflow while trying to print a stack trace,
 * which would make life very hard for us when we're debugging crashes for
 * customers. It would also be bad to crash from stack overflow when printing
 * backtraces on demand (SERVER-33445).
 *
 * A better idea is to get rid of the "frame" struct idea. Instead, iterate stack
 * frames with unw_step while printing the JSON stack, then reset the cursor and
 * iterate with unw_step again while printing the human-readable stack. Then
 * there's no need for a large amount of storage.
 *
...
void printStackTrace(std::ostream& os) {
    ...
    // Get the backtrace.
    ////////////////////////////////////////////////////////////
    size_t nFrames = getFrames(frames, os);
    if (!nFrames) {
